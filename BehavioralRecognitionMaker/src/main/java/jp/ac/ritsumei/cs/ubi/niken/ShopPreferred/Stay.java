...
public class Stay {
	...
	public void calcStay(){
		...
		ShoppingData.saveShoppingData(getClusteredShoppingData(), "C:\\data\\1year\\Output\\Vino\\P120427\\COVin120410.csv");
		//recalculate distance among truncates.
		
		recluster(getClusteredShoppingData());
		ShoppingData.saveShoppingData(getClusteredShoppingData(), "C:\\data\\1year\\Output\\Vino\\P120427\\RCOVin120410.csv");
	}
	
/*	private double getNumberCluster(ArrayList<ShoppingData> number){
		double numberCluster = 0;
		for (int i = 0; i < number.size();i++){
			if (i != number.size()-1) {
			ShoppingData data1 = number.get(i);
			ShoppingData data2 = number.get(i+1);
			
			String x1 = (String) data1.getTruncLat();
			String x11 = (String) data1.getTruncLong();
			String x2 = (String) data2.getTruncLat();
			String x22 = (String) data2.getTruncLong();
	...
	private boolean isSame(ShoppingData data1, ShoppingData data2) {
		if (data1.getTruncLat().equals(data2.getTruncLat()) && 
				data1.getTruncLong().equals(data2.getTruncLong())) {
			return true;
		}
		return false;
	}
